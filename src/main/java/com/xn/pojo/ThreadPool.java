...
public class ThreadPool {
    ...
    private int corePoolSize = 50;            //核心池的大小 运行线程的最大值 当线程池中的线程数目达到corePoolSize后，就会把多余的任务放到缓存队列当中；
    private int maximumPoolSize = 1000;        //创建线程最大值
    ...
    public static ThreadPool init() {
        if (threadPool == null || threadPool.isShutDowned())
    ...
    public void awaitTermination(int timeout, TimeUnit seconds) throws InterruptedException {
        getExecutor().awaitTermination(timeout, seconds);
    }
}